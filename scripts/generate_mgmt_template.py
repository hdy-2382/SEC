...
def build_guide(wb):
    ...
    notes = [
        ...
        ("조치검증", "에러에 대한 시정조치를 1건씩 기록. 검증시작Cycle = 조치 적용 시점의 누적 Cycle."),
...
def build_actions(wb):
    ...
    ws.sheet_properties.tabColor = NAVY
    cols = ["조치ID", "대상코드", "조치내용", "담당", "목표일", "상태", "검증시작Cycle"]
    _header(ws, cols, [10, 12, 40, 12, 13, 12, 14])
    ex = [
        ["A-001", "ERR-001", "카메라 버퍼 설정 변경", "양희두", "2026-06-20", "진행중", 358],
        ["A-002", "ERR-003", "경로 플래너 펌웨어 업데이트", "김현일", "2026-06-14", "완료", 200],
